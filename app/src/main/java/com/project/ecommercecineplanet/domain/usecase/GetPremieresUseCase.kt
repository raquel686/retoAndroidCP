...
class GetPremieresUseCase  @Inject constructor(private val repository: CineplanetRepository){
    operator fun invoke(): Flow<Resource<List<PremiereModel>>> {
        return flow {
            try {
                emit(Resource.Loading())
                val list=repository.getPremieresFromApi()
                if (list.isNotEmpty()){
